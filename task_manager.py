...
class TaskManager:
    ...
    def add_task(self, description: str, priority: str = "medium", due_date: str = None, category: str = "general") -> bool:
        """Add a new task with optional due date and category."""
        ...
        if due_date:
            try:
                ...
                for date_format in ["%Y-%m-%d", "%m/%d/%Y", "%d/%m/%Y"]:
                    try:
                        parsed_due_date = datetime.strptime(
                            due_date, date_format).isoformat()
                        break
                    except ValueError:
                        continue
                ...
                if not parsed_due_date:
                    print(
                        "Error: Due date must be in YYYY-MM-DD, MM/DD/YYYY, or DD/MM/YYYY format.")
                    return False
    ...
    def list_tasks(self, show_completed: bool = False, category_filter: str = None, priority_filter: str = None, due_date_filter: str = None) -> None:
        """List all tasks with due date and category information."""
        if not self.tasks:
            print("📝 No tasks found. Add some tasks to get started!")
            return

        filtered_tasks = [
            task for task in self.tasks if show_completed or not task["completed"]]

        # Filter by category if specified
        if category_filter:
            filtered_tasks = [task for task in filtered_tasks if task.get(
                "category", "general") == category_filter.lower()]

        if not filtered_tasks:
            print(
                "🎉 All tasks are completed!" if not category_filter else f"📝 No tasks found in category '{category_filter}'.")
            return
        ...
        for task in filtered_tasks:
            ...
            print(
                f"{status} [{task['id']}] {priority_emoji} {category_emoji} {task['description']}{due_info}")
            if task["completed"]:
...
Commands:
  add <description> [priority] [due_date] [category] - Add a new task
...
Examples:
  add "Buy groceries" high 2024-01-15 shopping
  add "Finish report" medium 01/20/2024 work
    ...
    while True:
        try:
            ...
            if cmd in ['quit', 'exit']:
                ...
                # Find priority, due date, and category in arguments
                for i, arg in enumerate(args):
                    if arg in ['high', 'medium', 'low']:
                        priority = arg
                    elif arg in ['work', 'personal', 'shopping', 'health', 'general']:
                        category = arg
                ...
                if due_tasks:
                    ...
                    for task in due_tasks:
                        ...
                        if days_overdue > 0:
                            print(
                                f"🔴 [{task['id']}] {task['description']} (OVERDUE by {days_overdue} days)")
                        else:
                            print(
                                f"🟡 [{task['id']}] {task['description']} (DUE TODAY)")
                else:
                ...
                if upcoming_tasks:
                    ...
                    for task in upcoming_tasks:
                        ...
                        print(
                            f"📅 [{task['id']}] {task['description']} (Due in {days_until} days)")
                else:
                ...
                task_manager.list_tasks(category_filter=command[1])
            elif cmd == 'priority':
                if len(command) != 2:
                    print("❌ Usage: priority <priority>")
                    continue

                task_manager.list_tasks(priority_filter=command[1])

            elif cmd == 'due_date':
                if len(command) != 2:
                    print("❌ Usage: due_date <due_date>")
                    continue

                task_manager.list_tasks(due_date_filter=command[1])
